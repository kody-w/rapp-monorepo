...
export function agentAvailability(agent: Agent, providers: Provider[], snapshot: Snapshot): string {
  if (agent.retiredAt) return "Retired";
...
export function Agents({ snapshot, providers, edit, create, connected, open, canCreate = true }: {
  snapshot: Snapshot; providers: Provider[]; edit: (agent: Agent) => void; create: () => void; connected: boolean;
  open: (agent: Agent) => void; canCreate?: boolean;
}) {
  ...
  return <section className="workspace-panel">
    ...
    {!agents.length ? <Empty icon="agents" title={snapshot.agents.length ? "No matching agents" : "Build a team with clear responsibilities"} action={!snapshot.agents.length && <button className="button primary" onClick={create} disabled={!connected || !canCreate}><Icon name="plus" size={16} />Create your first agent</button>}>
      ...
      <h3><button className="text-button" disabled={!connected} onClick={() => open(agent)}>{agent.name}</button></h3><p className="muted">{agent.role || "No role specified"}</p><p className="clamp agent-instructions">{agent.instructions || "No instructions configured."}</p>
      <p className="mono small-text" title="This agent's mint-once child workspace">Child workspace · {agent.workspaceId}</p>
      ...
      <button className="button primary" aria-label={`Open ${agent.name}'s workspace`} disabled={!connected} onClick={() => open(agent)}>Open child workspace<Icon name="arrow" size={16} /></button>
      <button className="button secondary" disabled={!connected || Boolean(agent.retiredAt)} onClick={() => edit(agent)}>Configure agent<Icon name="settings" size={16} /></button>
    </article>)}</div>}
  </section>;
}
