...
export function describeCadence(automation: Pick<Automation, "cadence">): string {
...
export function Automations({ snapshot, edit, create, connected }: {
  ...
  return <section className="workspace-panel">
    ...
    {!snapshot.automations.length ? <Empty icon="automations" title="Build a dependable routine" action={<button className="button primary" disabled={!connected} onClick={create}><Icon name="plus" size={16} />Create a routine with Twin</button>}>
      Describe what should happen and when. Your Twin drafts the complete routine and asks follow-up questions when needed.
      ...
      <div className="row between wrap"><p className="field-help">Local routines require RAPP Work and its runtime to remain available.</p><button className="button secondary" disabled={!connected} onClick={() => edit(automation)}>Review routine<Icon name="arrow" size={16} /></button></div>
    </article>)}</div>}
  </section>;
}
