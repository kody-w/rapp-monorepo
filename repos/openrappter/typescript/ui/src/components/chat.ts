...
export class OpenRappterChat extends LitElement {
  @property({ attribute: false }) initialSessionId: string | null = null;

  protected willUpdate(changes: PropertyValues) {
    if (changes.has('initialSessionId') && this.initialSessionId) {
      void this.switchSession(this.initialSessionId);
    }
  }
  ...
  private renderMessageGroup(group: MessageGroup) {
    ...
    const avatar = group.role === 'user' ? 'Y' : 'RW';
            ...
            ${groups.length === 0
              ? html`
                  <div class="empty-state">
                    <div class="empty-state-icon" aria-hidden="true">RW</div>
                    <div class="empty-state-text">
                      Start work with RAPP Work.<br />
                      Describe the outcome. Review the decisions.
                    </div>
                  </div>
                `
          ...
          <div class="input-area">
            ...
            <div class="input-container">
              <select
                ...
                ?disabled=${this.sessionTransitioning}
              >
                <option value="openrappter">RAPP Work</option>
                <option value="brainstem">Brainstem (compatibility)</option>
              </select>
              <button
                class="attach-btn"
