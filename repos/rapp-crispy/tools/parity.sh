...
head_ "retired egg — not regenerated for native releases"
if [ "${1:-}" = "--legacy-egg" ]; then
  if python3 - "$AGENT" "$mv" <<'PY'
import json, pathlib, sys, zipfile
with zipfile.ZipFile("rapp_crispy/eggs/rapp_crispy.egg") as archive:
    assert archive.read("twin/agents/rapp_crispy_agent.py") == pathlib.Path(sys.argv[1]).read_bytes(), "retired egg differs from current source"
    assert json.loads(archive.read("EGG.json"))["version"] == sys.argv[2], "retired egg version differs"
PY
  then ok "legacy egg matches (explicit archive check)"
  else bad "legacy egg differs; it is retired and must not be regenerated by the native release"; fi
elif [ -z "${1:-}" ]; then
  printf '       SKIP retired egg content parity; native app sources and source adapters are the active implementation.\n'
else
...
assert version == project == core == "1.5.0", (version, project, core)
PY
then ok "native plist, XcodeGen and core agree (1.5.0)"
