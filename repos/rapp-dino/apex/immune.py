...
# A RAPP dino is not a fork — each clone's body is ITS OWN owner, read from the
# organism config, never hardcoded to the hatchery's author. (The shared RAPP
# kernel/canon below is protected in every dino regardless of who owns it.)
from . import organism as _organism   # noqa: E402


def _self_owners():
    """The owners that ARE this organism's body. The clone's own owner, always."""
    return {_organism.owner()}


# The shared RAPP kernel + canon — protected in EVERY dino, whoever cloned it,
# because these are the common spine the whole species depends on. Naming them
# makes the intent auditable and survives an owner rename.
CANON_REPOS = (
    "kody-w/rapp-1",
    "kody-w/rapp-spine",
    "kody-w/rapp-map",
...
def check(candidate, roster=None):
    ...
    if full and full in (r.lower() for r in CANON_REPOS):
        return Verdict(True, "body", f"{full} is a shared canon/kernel repo (protected)")
    if owner in _self_owners():
        return Verdict(True, "body", f"owner {owner!r} is this organism itself (self)")
