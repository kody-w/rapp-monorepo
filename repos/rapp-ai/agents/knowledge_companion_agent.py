...
def _score_chunks(question, chunks):
...
class KnowledgeCompanionAgent(BasicAgent):
    ...
    def perform(self, **kwargs):
        ...
        return json.dumps({
            ...
            "passages": passages,
            "instruction": (
                "Answer the question using ONLY these passages. Cite each fact's source document "
                "by title (e.g. 'per the Workstream Directory'). Compose across passages when the "
                "answer spans sources (e.g. owner + escalation backup). Keep the answer under "
