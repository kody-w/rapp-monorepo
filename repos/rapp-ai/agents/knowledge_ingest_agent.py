...
def _ingest_sharepoint(library_url):
    library = json.loads(_fetch(library_url))
    chunks = []
    for doc in library.get("value", []):
        ...
        try:
            ...
            logging.warning(f"KnowledgeIngest: failed to fetch {raw_url}: {e}")
            continue
        chunks.extend(_chunk_markdown(doc, content))
    return chunks, len(library.get("value", []))
