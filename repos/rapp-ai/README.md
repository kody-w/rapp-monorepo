...
## 🏭 Included example: S/4HANA Transformation Knowledge Companion

Two drop-in agents (`agents/knowledge_ingest_agent.py`, `agents/knowledge_companion_agent.py`)
turn the platform into a grounded, citation-first knowledge companion for an ERP transformation
program. They ingest **live synthetic enterprise data sources** — an SAP-shaped OData gateway and
a SharePoint-shaped document library served entirely from GitHub raw:

```bash
RAW=https://raw.githubusercontent.com/kody-w/rapp-static-apis/main
curl -s $RAW/sap/registry.json | python3 -m json.tool          # the SAP sim
curl -s $RAW/sharepoint/api/v1/sites/phoenix/documents.json    # the program library
```

Try it (retrieval only, no keys needed):

```bash
python tests/e2e_knowledge_companion.py --dry
```

Repoint the two source URLs at a real SAP gateway + SharePoint site and the same agents ground on
production data. All bundled demo data is fully synthetic ("Meridian Manufacturing Group" /
"Project Phoenix" — every person and company fictional).
