...
the thing being acted upon. Both halves of that rule are implemented here:
  ...
  1. An automated actor never contributes to a human counter. The machine
     ...
     subtracted from the human comment count. This script adds no reaction
     anywhere — it has no reaction mutation at all, so it *cannot* move a
...
KNOWN LIMITS, stated rather than discovered later:
  * Comments are read one page deep (``comments(first: 100)``). GitHub
    returns discussion comments oldest-first and every machinery comment is
    created at seed time, so the markers sit at the head of the thread and
    stay visible; a thread that ever exceeds 100 comments could still hide
    an *editorial* comment (created later), which would make the editorial
    command append a second one instead of updating the first.
...
def human_comment_count(node: dict) -> int:
    ...
    return {
        ...
        "counting": {
            ...
            "comments": (
                ...
                "comments actually present on it."
            ),
            "signals": (
