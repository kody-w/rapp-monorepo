...
> **Status note on the citation above (NON-NORMATIVE; changes no rule).** `kody-w/rapp-eternity/SPEC.md` was retired to a tombstone on 2026-07-17 and now reads **RETIRED · SUBORDINATE TO RAPP · NON-NORMATIVE** — *"normative words in the historical version have no present authority."* The identity rule stated above is therefore **self-contained in this document** and is unchanged by that retirement; §6.1 states the same rule. It is **not** RAPP/1 §6 conformant on two counts: RAPP §6.2 mints the tail **once** from `Hb("rapp/1:rappid", …)` rather than deriving it from content, and RAPP §6.1 treats the bare-slug `rappid:<slug>:<hash>` form as legacy. Whether `rapp-moment/1.0` adopts RAPP §6 or remains deliberately divergent is an **open decision**, tracked in kody-w/rapp-moment#2 — not settled here.
...
A Moment's rappid is the **PKI-free SHA-256 content-address of its canonical body** — **never** `sha256("moment:"+pk)` and **never** key-derived, per the identity rule at the head of this document, which **supersedes** the key-derived minting rule formerly specified in this section. The canonical string form is `rappid:<slug>:<64hex>` (256-bit). The id is deterministic and eternal; the **hash is the join key**. The superseded derivations — `sha256("moment:"+pk)` for a Moment and `rappid:keeper:<sha256("keeper:"+pubx)>` for a zookeeper — are **legacy**: read-accepted under the compatibility contract below, **never emitted**. A zookeeper is identified by the `pub.x` fingerprint of its key (§6), which is an optional *ownership proof*, not an identity. Per the eternity compatibility contract, implementations **MUST read all legacy forms forever** (a `pk`, a bare UTC ms, a `|` separator), **emit only canonical**, and **never version the string — add record fields instead**. `sig_suite` (e.g. `ecdsa-p256`) is the crypto-agility field, covered by the signature.
