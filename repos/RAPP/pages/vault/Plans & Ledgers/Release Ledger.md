...
- **Scope.** New living doc at `pages/vault/Plans & Ledgers/Invention Backlog.md` — a generic capture framework for candidate inventions, plus a public-vault discipline section stating that implementation details, claim language, prior-art memos, and commercial numbers stay in private working docs; this file is the public *index*. Wired into `_manifest.json` and `_index.md` as the 7th living doc.
- **Why.** A vault-side capture surface ties the work to the platform's existing memory discipline. Public framework + private specifics is the right shape for IP-adjacent work — the framework is reusable, the specifics are not ours to publish.
...
- **Lesson.** The platform's vault discipline already separates *what's safe to publish* from *what's still hot*. Extending it to IP-adjacent work is a small step: same append-only ledger pattern, same session-pointer convention, same wikilink graph — with a clear *public-vault discipline* line drawn around anything substantive.
