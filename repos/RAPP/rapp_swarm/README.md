...
This file preserves the complete historical Tier-2 README from commit
`669c3f5518d92c989c5770c08e04b84e4c382294` as data exhaust. The commands,
routes, credentials, T2T surface, deployment claims, and moving references
below describe that historical implementation; they are not current RAPP/1
authority or active instructions.

The restored source now defaults to deterministic inspection, planning, or
local sandbox output with zero effects. Deployment, publication, extraction,
and mutation paths require reviewed dependency injection, exact target
receipts, and authenticated fresh section-13 owner evidence. That evidence is
not available in this repository, so effect requests refuse before transport
or mutation. Current chat adaptation routes only through the loopback
`http://127.0.0.1:7073/chat` facade. See
[`RAPP1_DEPLOYMENT_GUARD.json`](./RAPP1_DEPLOYMENT_GUARD.json) and
