...
Current no-flag behavior emits a deterministic in-memory observation. The
historical filesystem writer and optional ecosystem pulse are retained behind
reviewed injected executors, exact target receipts, and section-13 evidence.
...
HISTORICAL_SOURCE = {
    "path": "tools/sim/observe.py",
    "commit": "55b91b9ecd182a3ce2057787f07c60e9aa3ca128",
    "blob": "5539942887c866bfd0cdc087ad68cf77a0c39b8f",
    "sha256": "b2f29a703daa82d5d738df2b31f2915a874dfae373dfa043d2a31b890c7c54db",
    "bytes": 15513,
}
...
def _historical_call_bond_rhythm() -> dict | None:
...
def call_bond_rhythm(pulse: Callable[[], dict] | None = None) -> dict | None:
    """Run only an explicitly injected, reviewed pulse implementation."""
    if not isinstance(pulse, Callable):
        return {
            "_error": (
                "ecosystem pulse requires reviewed dependency injection; "
                "the historical importer is retained but not called by default"
            )
        }
    return pulse()
...
def _observer_parser() -> argparse.ArgumentParser:
...
def _parse_observer_args(argv):
    out_dir_count = sum(
        argument == "--out-dir" or argument.startswith("--out-dir=")
        for argument in argv
    )
    if out_dir_count > 1:
        raise ValueError("--out-dir may be supplied only once")
    return _observer_parser().parse_args(argv)


def _historical_main(argv=None, parsed_args=None):
    args = parsed_args if parsed_args is not None else _parse_observer_args(argv)
...
def sandbox_observation() -> dict:
    observation = {
        ...
        "measured": {
            ...
            "contributor_count": 2,
            "contributors": ["Alice", "Bill"],
            "total_submissions": 1,
            "total_votes": 1,
            "remix_count": 0,
            "voice_distinctiveness": 1.0,
            "per_twin": [],
            "antipattern_violations": [],
            "ecosystem_pulse": None,
        },
        "expected": {
            "minimum_contributors_after_first_tick": 2,
            "minimum_total_submissions_after_2_ticks": 1,
            "maximum_idle_seconds_per_twin": 3600,
            "minimum_voice_distinctiveness": 0.5,
        },
        "north_star": "deterministic local sandbox replay",
        "adjustments": [],
        "mode": "sandbox",
        "effects": [],
...
def run_observer(
    ...
    *,
    ...
    try:
        args = _parse_observer_args(argv)
    except (SystemExit, ValueError) as exc:
        return {
            "schema": "rapp-simulation-observer-result/1.0",
            "ok": False,
            "written": False,
    ...
    target = {
        "sim_root": SIM_ROOT,
        "out_dir": out_dir,
        "ecosystem_pulse": args.with_ecosystem_pulse,
    }
    refusal = authorize_effect(
        operation="simulation-observation-write",
        target=target,
    ...
    executor = dependencies.get("observer_executor")
    if not isinstance(executor, Callable):
        return {
            ...
            "error": {
                ...
                "step": "observer-executor",
            },
            "target": target,
        }
    executor(_historical_main, args, target)
    return {
        "schema": "rapp-simulation-observer-result/1.0",
        "ok": True,
        "written": True,
...
def main(argv=None):
    ...
    if not argv or argv in (["--sandbox"], ["--inspect"], ["--check"], ["--plan"]):
        print(json.dumps(sandbox_observation(), indent=2, sort_keys=True))
        return 0

    apply_requested = "--apply" in argv
    historical_argv = [arg for arg in argv if arg != "--apply"]
    if apply_requested or "--with-ecosystem-pulse" in historical_argv or "--out-dir" in historical_argv:
        result = run_observer(historical_argv)
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0 if result["ok"] else 78

    if "-h" in argv or "--help" in argv:
        print(
            "Usage: observe.py [--sandbox|--inspect|--check|--plan] "
            "[--apply --out-dir DIR] [--with-ecosystem-pulse]\n"
            "Default: deterministic in-memory observation with no reads or writes.\n"
            "Apply/pulse modes require reviewed dependency injection, an exact "
