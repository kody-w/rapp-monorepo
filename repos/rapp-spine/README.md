...
`registry.json` is generated from the spine's explicit source records and the
spec repositories themselves. The former `rapp-god`/`rapp-map` byte-identical
ecosystem mirror contract is retired: `rapp-map` serves a quarantine status
document, `rapp-god` is private, and RAPP-Bible is historical. Protocol
authority resolves to the exact immutable `kody-w/rapp-1` rev-5 pin.
