...
def content_digest(item: dict) -> str:
    ...
    if isinstance(item.get("recipe"), dict):
        basis["recipe"] = item["recipe"]  # a carried body is part of the record
...
#   2 — recipes carried: licensed entries run the upstream recipe verbatim
#       (prompt, prerequisites, steps, expected output) with attribution, and
#       model refinements from the local Brainstem are rendered when cached.
TOAST_GENERATION = 2
...
def render(
    ...
    *,
    ...
    refinement = spec.get("refinement") or {}
    if refinement.get("description"):
        desc = refinement["description"]
    if recipe:
        method_para = (
        ...
        method_para = (
            f"Nothing from upstream is reproduced here. What runs below is RAR's own method\n"
            f"for this shape of work — a {archetype} capability — generated from the metadata\n"
