...
code. The canonical body digests to:
    ...
    sha256 = d507eb26f76d5fd27a652e3bf9c2d8db09195dda68307455934d620ad15cf74f

If your copy differs, you are not running what the registry published. That
check turns skill drift from invisible into detectable.
"""
...
def md_to_html(text: str) -> str:
    ...
    for raw in re.split(r"\n{2,}", text.strip()):
        ...
        if all(re.match(r"^\s*\d+\.\s", l) for l in b.splitlines()):
            items = "".join(f"<li>{inline(re.sub(r'^\s*\d+\.\s+', '', l))}</li>" for l in b.splitlines())
